...
var RR = {
  ...
  Observable: {
    ...
    createAction: function() {
      ...
      if (args.length < 1) { throw 'not enougth arguments'; }

      if (argumentsAre(args, ['object'])) {
        var config = args[0];
        extend = Object.keys(config).reduce(function(ext, key) {
          var deps = getArgumentsNames(config[key]).map(RR._getObservable);
          ext[key] = config[key].apply(ext, deps);
          return ext;
        }, {});
...
function argumentsAre(args, types) {
  return types.map(function(type, idx) {
    if (type == 'object') {
      return !!args[idx] && !args[idx].length && typeof args[idx] == 'object';
    } else if (type == 'array') {
      return !!args[idx] && args[idx].length;
